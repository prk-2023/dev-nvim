...
require("lazy").setup({
  ...
  -- use snacks.nvim or fzf-lua : as telescope replacement 
  -- {
  -- 	"folke/snacks.nvim",
  -- 	priority = 1000,
  -- 	lazy = false,
  ...
  -- 		vim.keymap.set("n", "<leader>ss", function() Snacks.picker.pickers() end, { desc = "[S]earch Select Picker" })
  -- 		vim.keymap.set("n", "<leader>sw", function() Snacks.picker.word() end, { desc = "[S]earch current [W]ord" })
  ...
  -- }, 
  {
  	...
  	config = function()
  		...
  		vim.keymap.set("n", "<leader>ss", function() require("fzf-lua").builtin() end, { desc = "[S]earch [S]elect Fzf-Lua" })
  		vim.keymap.set("n", "<leader>sw", function() require("fzf-lua").grep_cword() end, { desc = "[S]earch current [W]ord" })
  		vim.keymap.set("n", "<leader>sg", function() require("fzf-lua").live_grep() end, { desc = "[S]earch by [G]rep" })
  		vim.keymap.set("n", "<leader>sd", function() require("fzf-lua").diagnostics_document() end, { desc = "[S]earch [D]iagnostics" })
  		vim.keymap.set("n", "<leader>sr", function() require("fzf-lua").resume() end, { desc = "[S]earch [R]esume" })
  		vim.keymap.set("n", "<leader>s.", function() require("fzf-lua").oldfiles() end, { desc = '[S]earch Recent Files ("." for repeat)' })
  		vim.keymap.set("n", "<leader><leader>", function() require("fzf-lua").buffers() end, { desc = "[ ] Find existing buffers" })
  		vim.keymap.set("n", "<leader>/", function() require("fzf-lua").blines() end, { desc = "[/] Fuzzily search in current buffer" })
  		vim.keymap.set("n", "<leader>s/", function() require("fzf-lua").live_grep({ grep_open_files = true }) end, { desc = "[S]earch [/] in Open Files" })
  		vim.keymap.set("n", "<leader>sn", function() require("fzf-lua").files({ cwd = vim.fn.stdpath("config") }) end, { desc = "[S]earch [N]eovim files" })
  	end,
  },
				...
				handlers = {
					function(server_name)
						if server_name == "rust_analyzer" then
							-- do nothing here , rustaceanvim will handle it
							return
						end
